...
- **第 1 层(地基):✅ 已完成(2026-06-24)**。主机 daemon(adapter 层 + SessionStore/QuotaTracker/Aggregator + 常驻 BLE)+ 固件 v7 TLV 解析 已合入 `main`(host 30 + 固件 native 4 测试全绿)。BLE OTA 刷入 v7 固件并完成真机 1b 验证:v7 编码 ↔ 解析 ↔ 显示与 v6 等价(5h/7d 额度、标题、倒计时正确)。daemon 使用见 `host/DAEMON.md` 。
  - 已知遗留(后续层处理):状态帧仍走 `0x07`(quota 才走 v7);v7 `title` UTF-8 截断;BLE OTA 慢(建议 WiFi OTA)。
- 第 2/3/4 层:待启动。
